...
**버전 1.3.3**
- 🔧 **복사 기능 수정**: 텍스트 복사가 정상적으로 작동하도록 수정
  - HTML 속성 이스케이핑 문제 해결 (따옴표, 줄바꿈 등 특수문자 처리)
  - data-text 속성 기반으로 안전한 복사 방식 변경
  - 이벤트 위임 방식으로 동적 생성 버튼 지원
- 📏 **텍스트 영역 높이 수정**: div 기반 텍스트 영역의 높이가 정상적으로 표시되도록 개선

**버전 1.3.2**
- 🧹 **코드 정리**: 다크테마 관련 불필요한 코드 및 하드코딩된 색상 완전 제거
  - 다크테마 주석 제거 (CSS 변수로 자동 처리되므로 불필요)
  - 모든 하드코딩된 색상(`rgba`, `#hex`)을 CSS 변수로 변경
  - 더 깔끔하고 유지보수하기 쉬운 코드 구조로 개선

**버전 1.3.1**
- 🎨 **UI 색상 개선**: 네비게이션 및 드롭다운 버튼이 채팅 백그라운드 색상을 사용하도록 변경
  - 기본 상태: 채팅 백그라운드 + 메인 텍스트 색상
  - 활성화(hover) 상태: 인용 텍스트 색상으로 변경하여 시각적 피드백 강화
  - SillyTavern 테마와 더욱 자연스럽게 통합
