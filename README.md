...
- 📝 **마크다운 렌더링**: AI가 생성한 마크다운 텍스트를 실시간으로 포맷팅해서 표시
...
- **마크다운 렌더링 영역**: 
  - **실시간 포맷팅**: 마크다운 문법을 실시간으로 렌더링해서 보기 좋게 표시
  - **지원 문법**: 헤딩, 굵은글씨, 기울임꼴, 코드블록, 인용문, 리스트, 테이블, 링크 등
  - **복사 버튼**: 각 텍스트 영역 우상단에 클립보드 복사 버튼 (원본 마크다운 형태로 복사)
  - **자동 높이 조정**: 내용 길이에 맞게 영역 크기 자동 조정, 스크롤 지원
...
### 마크다운 렌더링
- **실시간 포맷팅**: SillyTavern의 네이티브 마크다운 프로세서를 사용해 완벽 호환
- **지원 문법**: 
  - **텍스트 포맷팅**: **굵은 글씨**, *기울임꼴*, `인라인 코드`
  - **헤딩**: # ~ ###### 모든 레벨의 제목
  - **코드 블록**: 백틱(```) 코드 블록과 문법 강조
  - **인용문**: > 인용문 블록
  - **리스트**: 순서 있는/없는 리스트
  - **테이블**: 마크다운 테이블 지원
  - **링크**: 클릭 가능한 링크
- **테마 호환**: SillyTavern 테마에 맞는 색상과 스타일 자동 적용
- **원본 보존**: 복사 시 원본 마크다운 형태로 복사되어 포맷 유지
...
**버전 1.4.0**
- 📝 **마크다운 렌더링 지원**: AI가 생성한 마크다운을 실시간으로 포맷팅해서 표시
  - SillyTavern의 네이티브 마크다운 프로세서 활용으로 완벽한 호환성
  - 헤딩, 굵은글씨, 기울임꼴, 코드블록, 인용문, 리스트, 테이블, 링크 등 전체 마크다운 문법 지원
  - 텍스트필드 → 렌더링된 HTML 디스플레이로 변경하여 가독성 대폭 향상
- 📐 **레이아웃 최적화**: 텍스트 높이 자동 조정 및 스크롤 개선
  - 고정 높이 제거로 내용 길이에 맞는 동적 크기 조정
  - 화면 크기별 최적화된 최대 높이 설정 (데스크톱/태블릿/모바일)
  - 유연한 flex 레이아웃으로 텍스트 잘림 현상 완전 해결
- 📋 **복사 기능 유지**: 마크다운 형태 그대로 복사되어 다른 곳에 붙여넣기 시 포맷 보존
- 🎨 **시각적 개선**: SillyTavern 테마와 일치하는 마크다운 스타일링 적용

**버전 1.3.0**
- 🧹 **코드 클린업**: 디버깅용 코드 및 불필요한 복잡성 제거
  - 과도한 디버깅 로그 제거 (10+ 줄 → 필요 최소)
  - 임시 테스트 버튼 및 중복 이벤트 핸들러 제거
  - 복잡한 버튼 찾기 로직 단순화 (4가지 방법 → 1가지)
  - copyToClipboard 함수 간소화 및 성능 개선
- ✨ **사용자 경험 개선**: 복사 버튼 정상 작동 보장
  - 팝업 최초 생성 시 복사 버튼 이벤트 등록 문제 해결
  - 모든 시나리오에서 안정적인 복사 기능 제공
- 🔧 **코드 품질 향상**: async/await 일관성 및 에러 처리 개선
...
**버전 1.2.6-1.2.7 (개발)**
- 🔧 **복사 이벤트 처리 개선**: 이벤트 위임에서 직접 이벤트 등록 방식으로 변경
  - LALib 확장의 복사 방식을 참고하여 안정성 향상
  - HTML data 속성 제거, 순수 JavaScript 이벤트 핸들러 사용
  - 각 텍스트 영역별로 정확한 텍스트 복사 매핑
- 🐛 **복사 버튼 클릭 감지 개선**: setupCopyButtonEvents 함수 추가
  - 콘텐츠 업데이트 시마다 복사 버튼 이벤트 재등록
  - 더 상세한 디버깅 로그로 문제 진단 용이성 향상
- ⚡ **성능 최적화**: 불필요한 HTML 이스케이프 처리 제거

**버전 1.2.5**
- 🐛 **디버깅 로그 추가**: 복사 버튼 동작 문제 해결을 위한 상세 로그 추가
  - 함수 호출, 텍스트 처리, 클립보드 API 동작 상태를 콘솔에서 확인 가능
  - 이벤트 위임 방식으로 복사 버튼 처리 변경 (data 속성 사용)
  - HTML 특수문자 안전 처리 및 디코딩 로직 개선
- 🔧 **이벤트 처리 개선**: onclick 대신 이벤트 위임으로 더 안전한 복사 기능
- 📋 **안정성 향상**: 줄바꿈, 특수문자 등이 포함된 텍스트도 정상 복사
